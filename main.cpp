...
constexpr int D_FIXED = 20;
constexpr int POP_SIZE = 64;
constexpr int MAX_GENS = 1000000000;


constexpr double PA = 0.85;
constexpr double ABSOLUTE_GA_THRESHOLD = 1e-300;

constexpr int GA_POP_SIZE = 25;
constexpr double GA_RADIUS = 5.0;
constexpr double GA_MUTATION_STEP = 0.5;
constexpr int BITS_PER_VAR = 16;

unsigned long long function_calls = 0;
...
int main() {
    ...
    for (gen = 1; gen <= MAX_GENS; ++gen) {
        if (best_fit > ABSOLUTE_GA_THRESHOLD) {
            cout << "\n--- SLAITAS RASTAS! Perjungiama i GA. (Slenkstis: " << scientific << ABSOLUTE_GA_THRESHOLD << ") ---\n";
            ga_triggered = true;
            break;
        }
    ...
    chrono::duration<double> t_diff = end_t - start_t;
